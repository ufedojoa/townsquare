...
const Loader = ({ className }: { className?: string }) => {
  return (
    <svg
      ...
      className={
        (className ? className + " " : "") +
        "inline mr-3 h-full text-white animate-spin"
      }
      viewBox="0 0 100 101"
      fill="none"
...
export const SpaceCreateProposal = () => {
  ...
  const [endDate, setEndDate] = useState(
    new Date(Date.now() + 86400 * 1000 * 3)
  );
  ...
  const createProposal = useCallback(
    async ({
      ...
      const proposal = await client?.createProposal({
        ...
        title,
        spaceId: spaceId,
        end: BigInt(endDate.valueOf()) / 1000n,
        start: !startDate ? 0n : BigInt(startDate.valueOf()) / 1000n,
      });
      router.replace("/space/" + spaceId + "/proposals/" + proposal?.id);
    },
    [client, router]
  );
  ...
  return !space ? (
    ...
    >
      ...
      {!isSpaceAdmin &&
        settings &&
        settings !== "404" &&
        settings.createProposalThreshold > 0 && (
          <div className="">
            <p>
              The amount of {settings.createProposalThreshold}{" "}
              {space.token.symbol} will be charged upon creating this proposal.
              It&apos;ll be instantly refunded though.
            </p>
          </div>
        )}
      <PrimaryButton
        disabled={isLoading || !settings || !inSpace || formIsInvalid}
      >
        {isLoading ? (
          <>
      ...
      {!inSpace &&
        !(
          ...
          <div className="text-red-400">
            <p>
              You&apos;re not a member of this space. You need to be a member to
              create a proposal.
            </p>
          </div>
        )}
