...
import { DetailedSpace, Proposal, ProposalState, Space } from '@/types';
...
const RightSidebar = ({ proposal, space }: { proposal: Proposal; space: DetailedSpace }) => {
	const startDate = new Date(Number(proposal.start) * 1000);
	const endDate = new Date(Number(proposal.end) * 1000);
		...
		<div className="w-full lg:w-4/12 lg:min-w-[321px] space-y-4 md:space-y-6">
			<Block title="Information">
				<div className="space-y-2">
					...
					<SidebarTile label="Snapshot" value={proposal.snapshot.toString() ?? ''} />
				</div>
			</Block>
				...
				<div>
					...
					<div className="mb-4 flex flex-col sm:flex-row sm:space-x-1 text-skin-muted">
						<div className="mb-1 flex items-center sm:mb-0">
							...
							<Link className="group text-skin-text ml-3" href={'/space/' + space.id}>
...
const SingleProposal = () => {
	...
	const space = useSpace(BigInt(spaceId as string));
	const proposal = useProposal(BigInt(spaceId as string ), BigInt(proposalId as string));
